...
class Master extends EventEmitter {
     ...
     * @param {bunyan.Logger} [options.logger] bunyan instance
    ...
    constructor(options = {}) {
        ...
        if (options.log) {
            this.log = options.log.child({ component: 'master' });
        } else {
    ...
    readjustCluster() {
        ...
        Object.keys(cluster.workers).forEach(id => {
            ...
            if (worker.flora.state === 'running' || worker.flora.state === 'initializing' || worker.flora.state === 'forked') {
                activeWorkers++;
    ...
    async serverStatus() {
        ...
        Object.keys(cluster.workers).forEach(id => {
            ...
            if (!askWorker.isConnected()) {
                ...
                askWorker.flora.status.set('error', 'Worker already disconnected IPC channel, but is still running - status outdated');
                return;
            }
