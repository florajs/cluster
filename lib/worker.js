...
Worker.prototype.ready = function () {
    ...
    if (!cluster.isMaster) this.send({event: 'ready'});
};
...
Worker.prototype.shutdown = function () {
    ...
    try {
        ...
        var shutdownTimeout = setTimeout(function onShutdownTimeout() {
            self.logActiveHandles();
            self.log.warn('exit after timeout');
            process.exit(2);
...
Worker.prototype.logActiveHandles = function () {
    var self = this;

    if (typeof process._getActiveHandles !== 'function') {
        self.log.debug('Can\'t check for active handles - process._getActiveHandles not available');
        return;
    }
    ...
    if (cluster.worker.suicide && process._channel) whitelist.push(process._channel); // cluster IPC pipe

    var Socket = require('net').Socket;
    var Server = require('net').Server;
    var Timer = process.binding('timer_wrap').Timer; // internal node interface!
    var handles = process._getActiveHandles(); // internal node interface!

    var handleInfos = [];
    handles.forEach(function (handle) {
        var name = typeof handle;
        ...
        if (handle && typeof handle === 'object') {
            ...
            if (handle instanceof Socket) {
                description = handle.localAddress + ':' + handle.localPort +
                    ' -> ' + handle.remoteAddress + ':' + handle.remotePort +
                    (handle.destroyed ? ' (destroyed)' : '');
            } else if (handle instanceof Server) {
                var address = handle.address();
                description = address.address + ':' + address.port;
            } else if (handle instanceof Timer) {
                var timer = handle;
                while ((timer = timer._idleNext)) {
                    if (timer === handle) break;
                    if (timer._onTimeout) {
                        description = 'Timeout: ' + timer._idleTimeout;
                    }
                }
            } else {
                // ideas for additional generic description?
            }
        }

        handleInfos.push({name: name, description: description});
    });

    handleInfos.forEach(function (handleInfo) {
        self.log.warn('Active handle: ' + handleInfo.name +
            (handleInfo.description ? ' (' + handleInfo.description + ')' : ''));
    });
};
...
Worker.prototype.sendStatus = function () {
    ...
    if (!cluster.isMaster) this.send({event: 'status', status: status});
};
