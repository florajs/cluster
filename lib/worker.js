...
class Worker extends EventEmitter {
     ...
     * @param {http.Server} [options.httpServer] optional HTTP server instance
     */
    constructor(opts = {}) {
        super();
    ...
    logActiveHandles() {
        ...
        function dumpObjectProperties(obj) {
            ...
            do {
                ...
                properties.push(name + '[' + Object.getOwnPropertyNames(obj).join(', ') + ']');
            ...
            if (handle === null) {
                ...
                if (handle instanceof Socket) {
                    if (handle.remoteAddress) {
                        ...
                        description += ' -> ' + handle.remoteAddress + ':' + handle.remotePort;
                    } else {
                        description = dumpObjectProperties(handle);
                    }
                    description += handle.destroyed ? ' (destroyed)' : '';
                    ...
                    description += '; args: ' + (handle.spawnargs ? handle.spawnargs.join(' ') : '-');
                } else {
                    description = dumpObjectProperties(handle);
                }
            }
        ...
        handleInfos.forEach((handleInfo) => {
            this.log.warn(
                `Active handle: ${handleInfo.name}` + (handleInfo.description ? ` (${handleInfo.description})` : '')
            );
        });
    }
