...
function trackResponseEnd(request, response) {
    ...
    response.end = (...args) => {
        ...
        return this._floraOrigEnd(...args);
    };
...
class Worker extends EventEmitter {
    ...
    logActiveHandles() {
        ...
        if (cluster.worker.suicide && process._channel) {
        ...
        handleInfos.forEach((handleInfo) => {
            this.log.warn(`Active handle: ${handleInfo.name}` +
    ...
    serverStatus(callback) {
        ...
        serverStatusEvent.fetch();
        serverStatusEvent.once('serverStatus', status => callback(null, status));
    }
}
