...
serverStatusEvent.fetch = function () {
    process.send({event: 'serverStatus'});
};
...
function run(opts) {
    ...
    process.on('SIGTERM', function onSigTerm() {
        console.info('worker/%d: received SIGTERM, exiting immediately', process.pid);
        process.exit(0);
    });

    process.on('SIGINT', function onSigInt() {
        console.info('worker/%d: received SIGINT, exiting immediately', process.pid);
        process.exit(0);
    });

    process.on('SIGQUIT', function onSigQuit() {
        console.info('worker/%d: received SIGQUIT, shutting down gracefully', process.pid);
        shutdown();
    });
    ...
    process.on('uncaughtException', function onUncaughtException(err) {
        console.error('worker/%d: uncaught Exception, shutting down gracefully', process.pid);
...
function shutdown() {
    ...
    try {
        ...
        var shutdownTimeout = setTimeout(function onShutdownTimeout() {
            console.log('worker/%d: exit after timeout', process.pid);
            process.exit(1);
        ...
        console.error('worker/%d: error on graceful shutdown', process.pid);
...
function serverStatus(callback) {
    ...
    serverStatusEvent.once('serverStatus', function onServerStatus(serverStatus) {
